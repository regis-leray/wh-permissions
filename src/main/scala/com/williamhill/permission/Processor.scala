...
object Processor {
  ...
  val selfExclusionEventsSource: ZStream[Has[Config] & Has[Consumer], AppError, InputRecord] = {
    def mkStream(cfg: Config): ZStream[Any & Has[Consumer], AppError, InputRecord] =
      ZStream
        .fromEffect(
    ...
    for {
      cfg    <- ZStream.fromEffect(ZIO.service[Config])
      result <- mkStream(cfg)
    } yield result
  }
  ...
  val filterEventsFromSupportedUniverses: ZTransducer[Has[Config], AppError, InputRecord, InputRecord] =
  ...
  val calculatePermissions: ZTransducer[Any, AppError, FacetContextCommittable, FacetContextCommittable] =
    Committable.mapValueThrowable ( (facetContext:FacetContext) =>

      ZIO.succeed(PermissionsLogic.process(facetContext ))

    ).mapError(AppError.fromThrowable)

  val convertToFacetEvent: ZTransducer[ZEnv , AppError, FacetContextCommittable, OutputCommittable] =
    Committable.mapValueThrowable ( (facetContext:FacetContext) =>
      ZIO.succeed(OutputEvent(facetContext))
    ).mapError(AppError.fromThrowable)
  ...
  val publishEvent: ZTransducer[Has[EventPublisher] & Has[Config], AppError, OutputCommittable, OutputCommittable] = {
    ...
    Committable.mapValueM((event: OutputEvent) =>
      for {
  ...
  val commit: ZSink[Logging, AppError, OutputCommittable, OutputCommittable, Unit] =
    ZSink.foreach { record: OutputCommittable =>
      record.value.fold(
